...
main = do
  ...
  -- Check that 'config set' succeeds when applied to a stack.yaml file that
  -- does not use !include directives
  stack
    ["--stack-yaml","stack-not-including-flags.yaml","config","set","snapshot","ghc-9.8.4"]
